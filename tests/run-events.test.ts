...
test("consecutive tokens fold into one entry, and nothing else does", () => {
  events.emit("run-5", { kind: "output", text: "he" });
  events.emit("run-5", { kind: "output", text: "llo" });
  events.emit("run-5", { kind: "tool-call", name: "echo__ping", text: "{}" });
  events.emit("run-5", { kind: "output", text: "by" });
  events.emit("run-5", { kind: "output", text: "e" });

  const folded = events.fold(events.history("run-5"));
  expect(folded.map((event) => [event.kind, event.text])).toEqual([
    ["output", "hello"],
    ["tool-call", "{}"],
    ["output", "bye"],
  ]);

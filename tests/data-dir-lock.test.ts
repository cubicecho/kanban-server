...
function boot(dir: string): Promise<{ line: string; child: ChildProcess }> {
  const child = spawn(process.execPath, [FIXTURE], {
    ...
    env: { ...process.env, DATABASE_URL: "", KANBAN_SERVER_DATA_DIR: dir },
...
const temp = () => fs.mkdtempSync(path.join(os.tmpdir(), "kanban-server-lock-"));
...
test("a second server refuses the data directory the first one holds", async () => {
  ...
  expect(second.line).toMatch(/KANBAN_SERVER_DATA_DIR|DATABASE_URL/);
});

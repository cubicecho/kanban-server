 ...
 *
 * Stateless over HTTP: the descriptors are built once, and each request gets a server and a
 * transport of its own, answered as JSON. Nothing is pinned to a process and a client can
 * reconnect whenever it likes. Sessions would buy server-initiated messages over an open
 * stream; nothing here sends any.
 */
export const mcpHandler = createHttpHandler({
  ...
  // The prompts, on each server this mints and before it is connected — which is the one window
  // there is, since the SDK declares `capabilities.prompts` on the first registration and
  // refuses to declare one after a transport is attached. This file used to write out the
  // stateless handler by hand for exactly that reason, the driver handing its server to nobody;
  // cubicecho/graphql-mcp#20 landed as this hook and the fork went away with it.

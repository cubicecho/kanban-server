...
import type { RunUsage } from "@cubicecho/agent-core";
...
test("returns the reply and sums usage across turns", async () => {
  ...
  const result = await runAgent({ config: config(), systemPrompt: "be brief", prompt: "go" });
...
test("gives up once it has spent its tool iterations", async () => {
  ...
  await expect(runAgent({ config: config(), systemPrompt: "", prompt: "loop" })).rejects.toThrow(
    /3 tool iterations/,
  );
});
